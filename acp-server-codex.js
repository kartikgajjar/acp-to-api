...
// Remote binding safety gate: full-access agent must have auth when not on localhost
if (HOST !== '127.0.0.1' && HOST !== 'localhost' && ACP_API_KEYS.length === 0 && !ALLOW_INSECURE) {
  console.error(`[startup] ERROR: HOST=${HOST} with no ACP_API_KEY is unsafe for a full-access agent.`);
...
class ACPSession extends EventEmitter {
  ...
  async start() {
    const args = CODEX_ARGS ? CODEX_ARGS.split(' ').filter(Boolean) : [];
    this._proc = spawn(CODEX_CMD, args, {
      cwd:   CODEX_CWD,
    ...
    this._proc.once('exit', code => {
      ...
      for (const [, { reject }] of this._pending) reject(new Error(`codex-acp exited (${code})`));
      this._pending.clear();
      this.emit('dead');
    });
  ...
  _dbgLine(msg) {
    ...
    // Stderr: skip high-volume streaming noise and ping responses
    const m = msg.method;
    if (m === 'session/update' || m === 'session/notification') return;
    if (!m && (msg.error?.data === 'ping' || msg.error?.code === -32601)) return;
    ...
    if (msg.method === 'session/update' || msg.method === 'session/notification') {
      ...
      switch (type) {
        ...
        case 'AgentMessageChunk': {
          const text = payload.content?.text ?? payload.content ?? payload.text
                    ?? u.content?.text ?? u.content ?? u.text ?? '';
        ...
        case 'ToolCall':
          this.emit('chunk', { kind: 'thought', text: `[tool: ${payload.title ?? payload.name ?? u.title ?? u.name ?? 'unknown'}]\n` });
          break;
        case 'tool_call_update':
        case 'ToolCallUpdate': {
          const out = payload.output ?? payload.content?.text ?? u.output ?? u.content?.text;
          if (out) this.emit('chunk', { kind: 'thought', text: out });
          break;
        }
        case 'plan':
        case 'Plan': {
        ...
        case 'usage_update': {
          ...
          if (pt != null) this.emit('chunk', { kind: 'usage', promptTokens: pt, completionTokens: ct ?? 0 });
          break;
        }
  ...
  async initialize() {
    const result = await this._req('initialize', {
      protocolVersion: 1,
      clientInfo: { name: 'acp-codex-proxy', version: '1.0.0' },
    ...
    // Standard ACP expects the initialized notification (unlike Kiro which rejects it)
    this._send({ jsonrpc: '2.0', method: 'notifications/initialized' });
    this._startPing();
  }

  async newSession(cwd = CODEX_CWD) {
    ...
    // Set permission mode so agent runs without blocking on approval prompts
    await this._setMode(CODEX_MODE);
    return this.sessionId;
  }

  async _setMode(modeId) {
    if (!this.sessionId || !modeId) return;
    try {
      await this._req('session/set_mode', { sessionId: this.sessionId, modeId });
    } catch (e) { log(`mode:${this.label}`, `set_mode failed (${e.message})`); }
  }
  ...
  async setModel(modelId) {
    ...
    } catch (e) { log(`model:${this.label}`, `set_config_option failed (${e.message})`); }
  }
...
class ACPPool {
  ...
  async warmup() {
    log('pool', `warming ${this._size} codex-acp processes…`);
...
app.post('/v1/chat/completions', async (req, res) => {
  ...
  const cwd       = req.headers['x-working-dir'] ?? pickCwd(blocks, CODEX_CWD);
...
const server = app.listen(PORT, HOST, () => {
  if (_logStream) {
    ...
    const cfg = {
      ...
      CODEX_CMD, CODEX_CWD: CODEX_CWD.slice(0, 80),
      CODEX_MODE, CODEX_MODEL_DEFAULT, CODEX_AVAILABLE_MODELS,
  ...
  console.log(`
┌─────────────────────────────────────────────────────────────┐
│  ACP → OpenAI Proxy (Codex)  v1.0  —  http://${HOST}:${PORT}
│  Auth:    ${keyDisplay}
