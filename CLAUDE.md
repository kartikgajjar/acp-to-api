...
**Measured reality:** prefill is the floor and is mostly **fixed codex-acp agent-harness overhead** — a 10-token prompt still pays ~8 s prefill (the harness/tool system prompt OpenAI prefills each turn); `tools` push it higher (12–19 s observed with Langflow). User-prompt size is a minor term. This is why TTFT < 1 s is unreachable through codex-acp and warm-session reuse (~3 s) is the only large lever.
