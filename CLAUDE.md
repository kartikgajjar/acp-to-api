...
- **Session-not-found recovery (registry only)**: registry clients carry `_recoverSession = true`. If a `session/prompt` on a still-alive process is rejected as resource-not-found (an expired/unknown backend thread — code `-32001`/`-32002` or a `session … not found` message), `ACPSession.prompt` starts a fresh thread (`session/new`), re-applies the remembered model/effort (`_lastModel`/`_lastEffort`), and retries the turn **once** — guarded to fire only before any chunk has streamed. The stateless pool already creates a fresh `session/new` per request, so it never retries. `codex-appserver` does the analogous `thread/start` retry on a not-found `turn/start`.
...
- `MAX_EXEC_MS` enforced via `Promise.race([promptPromise, timeoutPromise])`. On timeout the proxy drains briefly (`REPLY_DRAIN_MS`, ~1 s): if the agent had already produced a (partial) reply it is returned with `finish_reason: 'length'` (chat) / a truncated `response` (responses) instead of failing; only when **no** output was produced does it cancel + 504. (Ollama defines `MAX_EXEC_MS` but does not enforce a per-request timeout, so this drain path is OpenAI-only.)
...
- Errors: OpenAI-envelope `{ error: { message, type, param, code } }` in the openai server; `{ error: <string> }` Ollama format in the ollama server. ACP/JSON-RPC failures are classified by a shared `classifyAcpError(err)` (mirrored in both interface files) that maps the preserved `err.acp.code` (set in `ACPSession._route`) to an HTTP status: `-32000` (+ auth text) → 401, `-32001`/`-32002` or `session … not found` → 404, `-32601`/`-32602` → 400, `-32603`/`-32700` → 503 (retryable), else 500. Best-effort calls that legitimately no-op (e.g. an unsupported `set_config_option` → `-32601`) are still swallowed and never reach this path.
